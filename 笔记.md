...
- **beforeCreate：**在数据渲染之前执行
- **created：**在数据渲染之前执行
- **beforeMount：**数据渲染之后执行
- **mounted：**数据渲染之后执行
- **beforeUpdate：**数据进行了修改，则会执行该方法
- **beforeDestroy：**销毁vue对象
- **destroy：**销毁vue对象
...
```vue
<!--
 * @Date: 2022-11-28 21:18:10
 * @LastEditors: wwz
 * @LastEditTime: 2022-11-28 21:21:05
 * @FilePath: \1010\vuedemo\11 vue生命周期.html
-->
<!DOCTYPE html>
<html lang="en">
...
将教程源文件\3-前端相关资源\前端资料\vue\vue-router.min.js复制到当前项目目录中gulixueyuan\1010\vuedemo

```vue
<!--
 * @Date: 2022-11-28 21:31:50
 * @LastEditors: wwz
 * @LastEditTime: 2022-11-28 21:58:38
 * @FilePath: \1010\vuedemo\12 vue路由.html
-->
<!DOCTYPE html>
<html lang="en">
...
<body>
    ...
    <!-- 引入顺序不能改变，先引入vue.min.js，再引入vue-router.min.js -->
    <script src="vue.min.js"></script>
...
![image-20221128220018324](http://typora-imagelist.oss-cn-qingdao.aliyuncs.com/image-20221128220018324.png)
...
![image-20221129211031348](http://typora-imagelist.oss-cn-qingdao.aliyuncs.com/image-20221129211031348.png)
...
2. 编写axios代码
   ...
   2. 使用axios发送请求，请求文件，得到数据，在页面显示

      ```vue
      <!--
       * @Date: 2022-11-29 21:14:03
       * @LastEditors: wwz
       * @LastEditTime: 2022-11-29 21:44:47
       * @FilePath: \1010\axiosdemo\01.html
      -->
      <!DOCTYPE html>
      <html lang="en">
          ...
          <script>
              new Vue({
                  ...
                  methods: {//编写具体的方法
                      ...
                      getUserList(){
                          //使用axios发送ajax请求
                          //axios.提交方式("请求接口路径").then(成功的回调函数).catch(失败的回调函数)
                          axios.get('data.json')
                          .then(response =>{
                              //response就是请求之后返回的数据
      ...
      ![image-20221129214907554](http://typora-imagelist.oss-cn-qingdao.aliyuncs.com/image-20221129214907554.png)
...
如果你是一个前端程序员，你不懂得像PHP、Python或Ruby等动态编程语言，然后你想创建自己的服
...
![image-20221130201938884](http://typora-imagelist.oss-cn-qingdao.aliyuncs.com/image-20221130201938884.png)



#### 22.9.4 使用node.js执行JavaScript代码
...
进入01.js文件的目录中，地址输入cmd，

![image-20221130203250009](http://typora-imagelist.oss-cn-qingdao.aliyuncs.com/image-20221130203250009.png)
...
![image-20221130203838559](http://typora-imagelist.oss-cn-qingdao.aliyuncs.com/image-20221130203838559.png)

在浏览器输入http://127.0.0.1:8888/

![image-20221130203916907](http://typora-imagelist.oss-cn-qingdao.aliyuncs.com/image-20221130203916907.png)
...
![image-20221130210153378](http://typora-imagelist.oss-cn-qingdao.aliyuncs.com/image-20221130210153378.png)

![image-20221130210224352](http://typora-imagelist.oss-cn-qingdao.aliyuncs.com/image-20221130210224352.png)
...
运行npmdemo的终端并输入`npm init`

![image-20221221215010839](http://typora-imagelist.oss-cn-qingdao.aliyuncs.com/image-20221221215010839.png)

执行一路回车键

![image-20221221215154352](http://typora-imagelist.oss-cn-qingdao.aliyuncs.com/image-20221221215154352.png)

此时npmdemo出现初始化的文件package.json

![image-20221221215230036](http://typora-imagelist.oss-cn-qingdao.aliyuncs.com/image-20221221215230036.png)

也可以使用`npm init -y`直接使用默认配置，无需多次回车键。
...
再执行`npm install --global babel-cli`

查看是否安装成功`babel --version`

**安装失败**：可能因为我的nodejs版本是8导致执行`npm install --global babel-cli`后，查看版本号出现错误

![image-20221222205453265](http://typora-imagelist.oss-cn-qingdao.aliyuncs.com/image-20221222205453265.png)

**解决方案：**

全局安装：执行`npm install --global @babel/core @babel/cli`

进入安装目录

![image-20221222210015891](http://typora-imagelist.oss-cn-qingdao.aliyuncs.com/image-20221222210015891.png)

重新打开vsCode

![image-20221222210129787](http://typora-imagelist.oss-cn-qingdao.aliyuncs.com/image-20221222210129787.png)

如果出现该图片

![202102181613611966427158](http://typora-imagelist.oss-cn-qingdao.aliyuncs.com/202102181613611966427158.png)
...
![image-20221222211321142](http://typora-imagelist.oss-cn-qingdao.aliyuncs.com/image-20221222211321142.png)



#### 23.3.5 转码

转码的方式有两种，一种是根据文件转码，一种是根据文件夹转码。
...
将01.js的代码转成es5放到dist文件夹中；

**根据文件命令操作：**

新建文件夹dist，执行命令`babel es6/01.js -o dist/001.js`

![image-20221222213020616](http://typora-imagelist.oss-cn-qingdao.aliyuncs.com/image-20221222213020616.png)



**根据文件夹转码操作：**

使用指令`babel es6 -d dist`



## 24. 模块化

![image-20221222214115642](http://typora-imagelist.oss-cn-qingdao.aliyuncs.com/image-20221222214115642.png)

### 24.1 es5模块化操作

> 新建moduledemo文件夹

初始化moduledemo文件夹，`npm init -y`

在moduledemo内创建es5module文件夹，再在es5module文件夹内创建01.js与02.js
...
在es5module文件夹内创建终端，执行`node 02.js`

![image-20221226203745985](http://typora-imagelist.oss-cn-qingdao.aliyuncs.com/image-20221226203745985.png)
